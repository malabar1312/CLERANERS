      ...
      <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
        {featuredReviews.slice(0, 3).map((review, i) => (
          <MotionReveal key={review.id} delay={i * 0.08}>
            <Card variant="white" padding="lg" className="relative flex h-full flex-col">
              <span
                className="font-display pointer-events-none absolute top-3 right-5 text-7xl leading-none text-[var(--color-surface-3)] select-none"
                aria-hidden="true"
              >
                &rdquo;
              </span>
              ...
              <figcaption className="mt-6 flex items-center gap-3 border-t border-[var(--color-line)] pt-5">
                <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--color-ink)] text-sm font-semibold text-[var(--color-white)]" aria-hidden="true">
