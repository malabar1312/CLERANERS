...
/**
 * `<CleanerCard />` — tarjeta de schoonmaker, noir editorial. La tarjeta
 * entera es un link al perfil (`/schoonmakers/[id]`).
 */
    ...
    <Link href={`/schoonmakers/${cleaner.id}`} className="block focus:outline-none" aria-label={cleaner.name}>
      <Card as="article" variant="noir" padding="md" interactive className="flex h-full flex-col">
        ...
        <div className="mt-5 flex items-end justify-between border-t border-[var(--color-line)] pt-4">
          ...
          <span className="inline-flex items-center gap-1.5 text-sm font-medium text-[var(--color-ivory-dim)] transition-colors group-hover/card:text-[var(--color-acid)]">
            {labels.viewProfile}
